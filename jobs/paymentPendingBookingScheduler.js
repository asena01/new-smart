...
export async function processPaymentPendingBookings(now = new Date()) {
  ...
  for (const booking of candidates) {
    // One malformed/legacy booking (e.g. missing createdAt, a bad hotelId populate) throwing
    // here used to abort the whole cycle — since that same record is never cancelled, it would
    // then keep blocking every OTHER unpaid booking's release on every future cycle too, not
    // just its own. Isolate each booking so a single bad record can't take the rest down with it.
    try {
      const hotel = booking.hotelId;
      if (!hotel) continue;
      const createdAt = booking.createdAt ? new Date(booking.createdAt) : null;
      if (!createdAt || Number.isNaN(createdAt.getTime())) {
        console.error(`Payment-pending scheduler: booking ${booking._id} has no valid createdAt, skipping`);
        continue;
      }
      const paymentDeadline = new Date(createdAt.getTime() + PAYMENT_GRACE_MS);
      ...
      console.error(`Payment-pending scheduler: failed to process booking ${booking._id}:`, error.message);
    }
  }
}
