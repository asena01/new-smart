...
function effectiveDeadline(booking, hotel) {
  // A paid reservation that never checked in is treated as a no-show once its entire
  // check-in DAY has passed — not at the policy's check-in START time (e.g. "14:00"), which
  // is just when the window opens, not a deadline. Guests routinely arrive well after that
  // hour (evening arrivals are completely normal); using it as the no-show cutoff was
  // auto-completing — and revoking the key for — a guest's brand-new multi-night stay before
  // they'd even reached the hotel, hours ahead of their actual, later arrival that same day.
  if (!booking.checkInInfo?.actualCheckInTime) {
    ...
    return combineDateAndTime(booking.checkInDate, '23:59', resolveHotelTimeZone(hotel));
  }
