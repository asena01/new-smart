...
const staffSchema = new mongoose.Schema({
  ...
  permissions: {
    ...
    // Remote lock/unlock via a room's TTLock gateway (not Bluetooth proximity) — e.g.
    // letting a receptionist in on a locked-out guest without walking to the door.
    canRemoteUnlock: { type: Boolean, default: false },
    // One-time per-lock Bluetooth provisioning for the offline hotel-card scheme (writing
    // hotelInfo + building/floor number into the lock) — see deviceController.getHotelCardInfo.
