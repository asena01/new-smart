...
import { recordDeviceAccessEvent, resolveActorName } from '../utils/deviceAccessEvents.js';
...
export const getHotelBookings = async (req, res) => {
  try {
    ...
    const operations = {
      // A no-show whose whole check-in day (plus grace period) has passed gets auto-completed by
      // jobs/autoCheckoutScheduler.js — checkOutInfo.actualCheckOutTime gets set, but
      // checkInInfo.actualCheckInTime never does, since the guest never checked in. Without the
      // actualCheckOutTime guard below, that booking satisfied the other two conditions forever
      // (checkInDate is always in the past, actualCheckInTime is always unset) and stayed stuck
      // in Arrivals with no way to act on it: Check In correctly refuses an already-checked-out
      // booking, and there was no Check Out action available on an Arrivals-bucket item at all.
      ...
      // Contactless check-in lets a guest complete ID verification up to 48h before their actual
      // checkInDate (see isWithinCheckInWindow), which sets actualCheckInTime immediately on
      // approval — well before the guest has physically arrived. Without the checkInDate guard
      // below, a guest who verified early for tomorrow's arrival showed up in "Currently
      // In-House" today. Requiring checkInDate to have arrived keeps this bucket meaning what its
      // label says; an early-verified guest simply doesn't appear in any bucket until arrival day.
...
export const getVerificationStatus = async (req, res) => {
  try {
    ...
    if (decision.status === 'Approved') {
      ...
      // isWithinCheckInWindow only compares calendar dates, so it doesn't by itself catch a
      // booking the auto-checkout scheduler already (possibly wrongly) closed out mid-stay —
      // same reasoning as the guard in confirmCheckIn above.
      if (booking.checkOutInfo?.actualCheckOutTime) {
        return res.status(409).json({ message: 'This booking has already been checked out. Contact the hotel if this seems wrong.' });
      }
...
export const getLockCredentials = async (req, res) => {
  try {
    ...
    // hasGateway is stored on the separate SmartLockDevice record (kept fresh by the admin
    // Test button and the background device monitor), not on the room's own
    // smartLockIntegration sub-document — this just tells the app whether to offer the
    // Remote Unlock button at all; the actual attempt still gets a live answer from TTLock.
    const device = await SmartLockDevice.findOne({ provider: 'ttlock', deviceId: eligible.smartLock.deviceId }).select('hasGateway');

    res.status(200).json({
      lockData: lockData.lockData,
      lockMac: lockData.lockMac,
      lockName: lockData.lockAlias || lockData.lockName,
      hasGateway: device?.hasGateway === true,
    });
  } catch (error) {
...
// Companion path to getLockCredentials above, for when the guest can't be at the door
// themselves (e.g. letting someone else in remotely) — same identity/stay-window
// enforcement, but issues the unlock straight from our backend through the gateway
// instead of handing the phone a Bluetooth credential. Requires hasGateway; without one
// there's no path for a remote command to reach the lock at all (see deviceMonitor.js).
export const remoteUnlockBooking = async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id).populate('hotelId').populate('userId', 'firstName lastName');
    if (!booking) {
      return res.status(404).json({ message: 'Booking not found' });
    }
    ...
    const eligible = getEligibleRoomLock(booking.hotelId, booking.roomId);
    if (!eligible) {
      return res.status(400).json({ message: 'Remote unlock is not available for this room' });
    }

    const device = await SmartLockDevice.findOne({ provider: 'ttlock', deviceId: eligible.smartLock.deviceId }).select('hasGateway');
    if (!device?.hasGateway) {
      return res.status(400).json({ message: 'Remote unlock is not available for this room right now — ask the front desk for help.' });
    }

    const result = await ttlockService.unlockDevice(eligible.smartLock.deviceId);
    if (result.errcode) {
      return res.status(502).json({ message: result.errmsg || 'Failed to unlock the door remotely' });
    }

    // isAuthorizedForBooking above allows the guest themselves OR staff/host/admin acting on
    // their behalf (front desk help), so the actor here isn't always the guest — resolve
    // whichever actually called this.
    const actorName = req.user.role === 'guest'
      ? (booking.userId ? `${booking.userId.firstName} ${booking.userId.lastName}` : (booking.guestName || null))
      : await resolveActorName(req);
    ...
    res.status(200).json({ message: 'Door unlocked remotely' });
  } catch (error) {
    console.error('Error unlocking booking remotely:', error.message);
    res.status(502).json({ message: error.message || 'Failed to unlock the door remotely' });
  }
};
...
export const confirmCheckIn = async (req, res) => {
  try {
    ...
    // Without this, a booking the auto-checkout scheduler already closed out (e.g. wrongly
    // flagged as a no-show — see effectiveDeadline in autoCheckoutScheduler.js) could still be
    // "checked in" here, which set actualCheckInTime and reset status back to 'confirmed'
    // while leaving the stale checkOutInfo.actualCheckOutTime in place — a booking that looks
    // simultaneously checked in and checked out, invisible to both the arrivals and in-house
    // lists on Front Desk. A genuinely mistaken auto-checkout needs an explicit host
    // correction (clearing checkOutInfo), not a silent overwrite via a second check-in.
    if (booking.checkOutInfo?.actualCheckOutTime) {
      return res.status(409).json({
        message: 'This booking has already been checked out. If this was a mistake, contact the host to reopen it before checking in again.'
      });
    }
