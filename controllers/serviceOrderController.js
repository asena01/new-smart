...
export const createServiceOrder = async (req, res) => {
  try {
    ...
    // Validate hotel exists (moved ahead of the check-in gate below, which needs it to resolve
    // the hotel's actual check-in time)
    const hotel = await Hotel.findById(hotelId);
    if (!hotel) {
      return res.status(404).json({ error: 'Hotel not found' });
    }

    // Confirmed just means the reservation/payment went through — hotel services are only
    // orderable once the guest has actually checked in (front-desk or contactless) AND the
    // hotel's actual check-in moment has arrived. That second part is the one that was missing:
    // contactless check-in can set actualCheckInTime up to 48h before that moment (see
    // isWithinCheckInWindow in bookingController.js), so a guest who completed remote ID
    // verification early got full access to restaurants/bar/every hotel service days before
    // their stay actually began — checking actualCheckInTime alone doesn't tell you when the
    // guest is actually due. Early check-in is the one exception: it's a fixed-fee paid service
    // (pay to have gotten in before the hotel's standard check-in time), so it's requestable both
    // before actualCheckInTime is set (the common case) and after (to formalize/pay for early
    // hours already used via the free check-in window) — see canRequestEarlyCheckIn in
    // bookingController.js for the cutoff that still applies either way.
    if (serviceType !== 'early-checkin') {
      if (!booking.checkInInfo?.actualCheckInTime) {
        return res.status(403).json({ error: 'Please check in before ordering hotel services.' });
      }

      const effectiveCheckInAt = booking.checkInInfo?.approvedEarlyCheckInTime
      ...
      if (new Date() < effectiveCheckInAt) {
        return res.status(403).json({
          error: `Hotel services aren't available until check-in time (${effectiveCheckInAt.toLocaleString()}).`
        });
      }
    }
