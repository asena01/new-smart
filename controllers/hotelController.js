...
export const getHotelById = async (req, res) => {
  try {
    const { checkIn, checkOut, guests, includeAllRooms } = req.query;
    ...
    const plain = hotel.toObject();
    // Admin/host management views (room-picker drawers for device assignment, room lists,
    // etc.) call this same public endpoint but need every room regardless of housekeeping or
    // maintenance status — a room being 'dirty' has nothing to do with whether a lock or
    // sensor should be attachable to it. Only guest-facing booking flows should ever see the
    // availability-filtered list, so this stays opt-in rather than flipping the default.
    plain.rooms = includeAllRooms === 'true'
      ? plain.rooms
      : await getAvailableRooms(hotel._id, plain.rooms, checkIn, checkOut, guests);
...
export const getHostDashboard = async (req, res) => {
  try {
    ...
    // room.status is host-set sellability intent, not occupancy — a checked-in guest never
    // changes it (see attachRoomOccupancy's own comment above). Without this, occupiedRooms
    // below only ever matched the legacy 'occupied' enum value, which nothing in the codebase
    // still writes, so it stayed 0 regardless of how many rooms actually had a guest checked in.
    hotel = await attachRoomOccupancy(hotel);
    ...
    res.status(200).json({
      ...
      metrics: {
        ...
        occupiedRooms: hotel.rooms?.filter(room => room.occupancyStatus === 'occupied').length || 0,
        availableRooms: hotel.rooms?.filter(room =>
          room.currentStatus === 'available' && ROOM_READY_STATUSES.includes(room.housekeepingStatus)
        ).length || 0,
        ...bookingMetrics,
        serviceOrders,
        serviceRevenue,
