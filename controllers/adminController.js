...
// Guests aren't tied to a hotel by any direct field — the only link is through their
// bookings there. Two shapes of guest exist: real accounts (Booking.userId set) and
// walk-in/no-account bookings (host-reception source, just guestName/guestEmail/guestPhone
// on the booking itself) — both need to show up here, but only the former can actually be
// edited/deleted as a User account, so hasAccount tells the frontend which is which.
// Grouped by account id (or, for walk-ins, lowercased email) so a repeat guest shows once
// with their full stay history, not once per booking. Shared by both the per-hotel and
// platform-wide guest lists below — bookingFilter is the only thing that differs between them.
async function buildGuestList(bookingFilter) {
  const bookings = await Booking.find(bookingFilter)
    .select('userId hotelId guestName guestEmail guestPhone checkInDate checkOutDate status')
    .populate('userId', 'firstName lastName email phone status identityVerificationStatus createdAt')
  ...
  for (const booking of bookings) {
    ...
    const key = account ? String(account._id) : (booking.guestEmail || booking.guestName || '').toLowerCase();
    if (!key) continue;
    ...
    if (!guestsByKey.has(key)) {
      guestsByKey.set(key, {
        ...
        hasAccount: !!account,
        firstName: account ? account.firstName : (booking.guestName?.split(' ')[0] || booking.guestName || 'Guest'),
        lastName: account ? account.lastName : (booking.guestName?.split(' ').slice(1).join(' ') || ''),
        email: account ? account.email : (booking.guestEmail || ''),
        phone: account ? account.phone : (booking.guestPhone || ''),
        status: account ? account.status : null,
        identityVerificationStatus: account ? account.identityVerificationStatus : null,
        bookingsCount: 0,
        lastStayDate: null,
        hotelNames: new Set()
      });
    ...
    const entry = guestsByKey.get(key);
    entry.bookingsCount += 1;
    if (!entry.lastStayDate || booking.checkInDate > entry.lastStayDate) {
      entry.lastStayDate = booking.checkInDate;
    }
    if (booking.hotelId?.name) entry.hotelNames.add(booking.hotelId.name);
  }

  return Array.from(guestsByKey.values()).map(g => ({ ...g, hotelNames: Array.from(g.hotelNames) }));
}

export const getHotelGuests = async (req, res) => {
  try {
    const guests = await buildGuestList({ hotelId: req.params.id });
    res.json({ guests });
  } catch (error) {
...
// Same guest shape as getHotelGuests, but across every hotel on the platform — hotelNames
// lets the frontend show "stayed at 3 hotels" for a repeat guest instead of just a count.
export const getAllGuests = async (req, res) => {
  try {
    const guests = await buildGuestList({});
    res.json({ guests });
  } catch (error) {
...
// Suspending blocks login (authController.js) and immediately kills any already-issued
// token (middleware/auth.js's protect) — separate from Hotel.isActive, which only turns a
// hotel's own listing on/off and leaves the host free to keep using the platform otherwise.
export const updateUserStatus = async (req, res) => {
  try {
    ...
    // Suspending another admin is deliberately blocked here — this endpoint has no notion of
    // a "super admin" tier, so nothing would stop two admins from locking each other out.
    if (user.role === 'admin') {
      return res.status(400).json({ message: "Admin accounts can't be suspended from here" });
    }
...
// Same two guardrails as the single-delete endpoint above, just applied per id instead of
// failing the whole request on the first bad one — a batch of 20 shouldn't get blocked
// entirely because one of them happens to be a host who still owns a hotel. Reports back
// which ones were actually deleted vs skipped (and why) so the admin isn't left guessing.
  ...
  try {
    ...
    for (const id of userIds) {
      if (id === req.user.userId) {
        skipped.push({ id, reason: "Can't delete your own account" });
        continue;
      }
      ...
      if (user.role === 'host') {
        const ownedHotel = await Hotel.findOne({ hostId: user._id });
        if (ownedHotel) {
          skipped.push({ id, reason: `Still owns "${ownedHotel.name}"` });
          continue;
        }
      }
    ...
    res.json({ deletedCount: deleted.length, deleted, skipped });
  } catch (error) {
...
// Everything the admin needs to actually create/manage a hotel's Flutterwave subaccount —
// deliberately its own endpoint, not folded into the public getHotelById, since that one
// explicitly strips bankDetails/flutterwaveSubaccountId before it ever reaches a guest.
export const getHotelPaymentDetails = async (req, res) => {
  try {
    const hotel = await Hotel.findById(req.params.id).select('bankDetails flutterwaveSubaccountId');
    if (!hotel) {
      return res.status(404).json({ message: 'Hotel not found' });
    }

    res.json({ bankDetails: hotel.bankDetails || null, flutterwaveSubaccountId: hotel.flutterwaveSubaccountId || null });
  } catch (error) {
    console.error('Error fetching hotel payment details:', error);
    res.status(500).json({ message: 'Failed to fetch hotel payment details' });
  }
};
